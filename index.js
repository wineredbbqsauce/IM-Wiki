...
function renderFeed() {
  ...
  if (older.length > 0) {
    feed.appendChild(makeDivider("Edlre Poster"));
...
function formatDate(ds) {
  return new Date(ds).toDateString("nb-NO", {
    day: "numeric",
    month: "long",
    year: "numeric",
    hour: "2-digit",
...
function updatePublishBtn() {
  ...
  document.getElementById("btnPublish").disable = !(t && c);
}
...
function saveEdit() {
  posts = posts.map((p) =>
    p.id !== editingId
      ...
      : {
          ...p,
          title: document.getElementById("editTitle").valuet.trim(),
...
function makeDivider(label) {
  const dih = document.getElementById("div");
